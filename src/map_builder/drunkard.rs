...
const NUM_TILES: usize = (WORLD_WIDTH * WORLD_WIDTH) as usize;
...
impl DrunkardWalkArchitect {
    fn drunkard(&mut self, start: &Point, rng: &mut RandomNumberGenerator, map: &mut Map) {
        ...
        loop {
            let drank_idx = map.point2d_to_index(drunkard_pos);
            map.tiles[drank_idx] = TileType::Floor;
...
impl MapArchitect for DrunkardWalkArchitect {
    fn new(&mut self, rng: &mut RandomNumberGenerator) -> MapBuilder {
        ...
        {
            self.drunkard(
                &Point::new(rng.range(0, WORLD_WIDTH), rng.range(0, WORLD_HEIGHT)),
                rng,
                &mut mb.map,
            );
